...
public interface ITaskRepository extends CrudRepository<Task, Long> {
    ...
    @Query("SELECT t FROM Task t JOIN Status s ON t.status.id = s.id JOIN Category c ON t.category.id = c.id WHERE (t.status.id = 1 or t.status.id = 2) and t.category.id = :category ORDER BY t.beginTask asc ")
